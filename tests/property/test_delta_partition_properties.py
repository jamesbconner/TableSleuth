...
def test_property_partition_skew_detection(
    ...
    skewed_partitions = [
        i for i, count in enumerate(files_per_partition)
...
@given(
    ...
    partition_types=st.lists(
        st.sampled_from(["string", "integer", "date", "timestamp"]),
        min_size=1,
        max_size=5
    )
)
def test_property_partition_metadata_display(
    ...
    partition_metadata = [
        {"name": col, "type": typ}
        for col, typ in zip(partition_columns, partition_types)
    ]
...
def test_property_partition_file_distribution(
    partition_values: list[tuple[str, int]]
) -> None:
