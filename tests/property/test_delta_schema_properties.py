...
@given(
    schema_v1_fields=st.lists(schema_field_strategy, min_size=1, max_size=10, unique_by=lambda x: x["name"]),
    schema_v2_fields=st.lists(schema_field_strategy, min_size=1, max_size=10, unique_by=lambda x: x["name"])
)
def test_property_schema_change_detection(
    schema_v1_fields: list[dict],
    schema_v2_fields: list[dict]
) -> None:
    ...
    type_changes = {
        ...
        if v1_fields[name] != v2_fields[name]
    }
...
@settings(max_examples=100)
@given(
    column_name=st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=("Lu", "Ll"))),
...
@settings(max_examples=100)
@given(
    schema_fields=st.lists(schema_field_strategy, min_size=1, max_size=10, unique_by=lambda x: x["name"]),
    columns_to_remove=st.lists(st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=("Lu", "Ll"))), min_size=0, max_size=5)
)
