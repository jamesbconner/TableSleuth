...
def test_property_amplification_trend_analysis(
    ...
    amplifications = [
        base_amplification + (i * trend_increase)
    ...
    expected_trend = (base_amplification + ((operation_count - 1) * trend_increase)) > (base_amplification * 2)
...
@settings(max_examples=100)
@given(
    amplifications=st.lists(
        st.floats(min_value=1.0, max_value=100.0),
        min_size=1,
        max_size=50
    )
)
def test_property_high_amplification_highlighting(
